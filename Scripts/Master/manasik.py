...
# -*- coding: utf-8 -*-
"""
manasik.py (fixed clean)
- Hapus escape karakter \" yang menyebabkan SyntaxError
- Tetap: baca JPG recursive, 1(1)/1(2) -> 1.psd, config.json opsional
"""
...
"""
                ...
                bat_file_path = os.path.join(final_output_folder, "snap.bat")
...
def find_candidate_codes(folder_path, prefer_tag=None):
    ...
    def push(x):
        if x is None: return
        x = str(x)
        # Filter out 8 if it looks like 8R, or 10 if 10RP
        if x not in seen:
            seen.add(x)
            cand.append(x)
    ...
    try:
        for filename in os.listdir(folder_path):
            ...
            # Gabung nama file dan isi untuk pencarian
            search_text = f"{filename}\n{content}"
            
            # Strategi: Ambil semua angka 1-3 digit
            raw_nums = re.findall(r'(?<!\d)(\d{1,3})(?!\d)', search_text)
            
            # A. Explicit MSK pattern: MSK[- ]?(\d{1,3})
            for m in re.finditer(r'MSK[\s-]*(\d{1,3})', search_text, re.IGNORECASE):
                push(m.group(1))

            # B. Explicit Type pattern: (10RP|8R)[\s-]*(\d{1,3})
            for m in re.finditer(r'(?:10RP|8R)[\s-]*(\d{1,3})', search_text, re.IGNORECASE):
                push(m.group(1))

            # C. Fallback: Ambil semua angka, tapi validasi
            for num_str in raw_nums:
                # Cek apakah angka ini adalah '8' dan di teks aslinya diikuti 'R'
                if num_str == '8' and '8R' in search_text.upper():
                    continue 
                if num_str == '10' and '10RP' in search_text.upper():
                    continue 
                push(num_str)
...
def find_master_file(master_folder, code):
    """Mencari file di master_folder yang nomornya COCOK EKSAD dengan code."""
    try:
        # Code harus angka
        cat_digits = re.sub(r'\D', '', code)
        if not cat_digits:
            return None
        target_num = int(cat_digits)
        ...
        for fname in os.listdir(master_folder):
            ...
            nums_in_file = re.findall(r'\d+', fname_no_ext)
            
            for num_str in nums_in_file:
...
def main(master_path_primary, pilihan_path, output_base_path, master_path_secondary=None):
    ...
    print(f"--- Memulai Proses Manasik ---")
    ...
    for subfolder_name in os.listdir(pilihan_path):
        ...
        if 'manasik' not in subfolder_lower:
            continue
        ...
        print(f"  - Kandidat kode dari .txt: {candidates if candidates else '∅'}")
        if not candidates:
