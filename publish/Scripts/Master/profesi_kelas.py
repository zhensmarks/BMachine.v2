...
CLASS_OR_GROUP_REGEX = re.compile(r'^(?:KELAS|KLS|GROUP|KELOMPOK)[\s._-]*', re.IGNORECASE)
...
def process_images(master_path_profesi, master_path_sporty, pilihan_path, output_path, config_data,
    ...
    for full_path in files:
        try:
            ...
            rel_dir = os.path.relpath(os.path.dirname(full_path), pilihan_path)
            parts_dir = [] if rel_dir in (".", "") else rel_dir.split(os.sep)

            kategori_folder = parts_dir[0].lower() if len(parts_dir) > 0 else ""
            sporty_mode = 'sporty' in kategori_folder
            profesi_mode = 'profesi' in kategori_folder

            final_label = None
            if (sporty_mode or profesi_mode) and len(parts_dir) >= 2:
                if is_class_or_group(parts_dir[1]):
                    if len(parts_dir) >= 3 and not is_class_or_group(parts_dir[2]):
                        final_label = parts_dir[2].strip()
                else:
                    final_label = parts_dir[1].strip()

            if not final_label:
                final_label = label_from_name

            if not final_label and parts_dir:
                tag_idx = next((i for i, seg in enumerate(parts_dir) if is_class_or_group(seg)), None)
                if tag_idx is not None and tag_idx + 1 < len(parts_dir):
                    cand = parts_dir[tag_idx + 1]
                    if not is_class_or_group(cand) and not re.fullmatch(r'\d+', cand):
                        final_label = cand

            if not final_label:
                for kw in ('sporty', 'profesi'):
                    idx = next((i for i, seg in enumerate(parts_dir) if kw in seg.lower()), None)
                    if idx is not None and idx + 1 < len(parts_dir):
                        cand = parts_dir[idx + 1]
                        if not is_class_or_group(cand):
                            final_label = cand
                            break

            if not final_label:
                print(f"NAMA_HILANG:{full_path}", file=sys.stderr)
                continue

            mapped = try_resolve_mapping(final_label, profession_mappings)
            if mapped:
                print(f"[MAP] '{final_label}' -> '{mapped}'")
                final_label = mapped

            category = 'sporty' if sporty_mode else ('profesi' if profesi_mode else detect_category_from_parts(parts_dir))
            use_profesi = (category != 'sporty')
            if (not use_profesi) and (not master_sporty_exists):
                unmatched.append(f"'{filename}' (label: '{final_label}', kategori: 'SPORTY' - master tidak tersedia)")
                continue

            master_dict = master_files_profesi if use_profesi else master_files_sporty
            master_root = master_path_profesi if use_profesi else master_path_sporty

            master_key = try_find_master_key(final_label, master_dict)
            if not master_key:
                vars_hint = ", ".join(generate_pf_variants(final_label))
                unmatched.append(f"'{filename}' (label: '{final_label}', kategori: '{'PROFESI' if use_profesi else 'SPORTY'}')")
                print(f"[WARN] MASTER tidak ditemukan untuk '{final_label}'. Coba varian: {vars_hint}", file=sys.stderr)
                continue

            file_master_name = master_dict[master_key]
            file_master_path = os.path.join(master_root, file_master_name)
            master_ext = os.path.splitext(file_master_name)[1]

            # Create output directory by mirroring the source structure
            output_dir = os.path.join(final_event_folder, rel_dir)
            os.makedirs(output_dir, exist_ok=True)

            tujuan_path = os.path.join(output_dir, f"{base_name}{master_ext}")
            if os.path.exists(tujuan_path):
                print(f"SKIP_EXISTING:{tujuan_path}")
                continue
